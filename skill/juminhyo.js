...
module.exports = class SkillJuminhyo {
    ...
    constructor(){
        this.required_parameter = {
            type: {
                ...
                parser: (value, bot, event, context, resolve, reject) => {
                    return parse.by_nlu(context.sender_language, "parse_juminhyo_type", value, resolve, reject);
                },
        ...
        this.optional_parameter = {
            is_name_correct: {
                ...
                parser: (value, bot, event, context, resolve, reject) => {
                    return parse.by_nlu(context.sender_language, "parse_yes_no", value, resolve, reject);
                },
                reaction: (error, value, bot, event, context, resolve, reject) => {
                    if (error) return resolve();
            ...
            street: {
                ...
                reaction: (error, value, bot, event, context, resolve, reject) => {
                    ...
                    if (bot.type === "google"){
                        context.confirmed.payment = "代引き";
                        return resolve();
                    }

                    bot.collect("payment");
                    bot.collect("social_id");
                    return resolve();
                }
            },
            address: {
            ...
            payment: {
                ...
                parser: (value, bot, event, context, resolve, reject) => {
                    return parse.by_nlu_with_list(context.sender_language, "parse_payment", value, ["LINE Pay", "代引き"], resolve, reject);
                }
            }
        }
    ...
    finish(bot, event, context, resolve, reject){
        ...
        if (context.confirmed.payment === "代引き"){
            let message = {
                type: "text",
                text: `住民票申請の受付を完了しました。2-3日中に身分証のご住所に住民票をお届けしますね。`
            }
            if (!context.confirmed.social_id){
                message.text += "その際、ご本人確認をさせていただきますので身分証明証を配達員にご提示ください。";
            }
        ...
        return bot.reply({
            type: "text",
            text: `住民票申請の受付を完了しました。手数料のお支払いが完了しましたらすぐにご自宅に住民票を発送します。`
        }).then((response) => {
            return line_event.fire({
                ...
                intent: {
                    ...
                    parameters: {
                        ...
                        message_text: `下記のボタンからお支払いをお願いします`
                    }
                },
